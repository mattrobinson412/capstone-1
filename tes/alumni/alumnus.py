...
def handle_alum_login():
    """Handle login function for an alumni user."""
    ...
    return render_template('/alumni/login.html', form=form)


# @alumni.route("/logout", methods=["GET", "POST"])
# @login_required
# def handle_alum_logout():
...
def show_alumni_homepage():
    ...
    if user.status.status != "Alumni":
        ...
        user = User.query.get(current_user.id)
        return render_template("alumni/home.html", user=user)


@alumni.route("/contact", methods=["GET", "POST"])
@login_required
def handle_alumni_contact_form():
    ...
    if user.status.status != "Alumni":
        ...
        if form.validate_on_submit():
            ...
            return render_template("alumni/contact.html", form=form)


@alumni.route("/settings", methods=["GET", "POST"])
@login_required
def change_alumni_settings():
    ...
    if user.status.status != "Alumni":
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            return redirect(f"/alumni/home")

        else:
            return render_template("/alumni/settings.html", form=form, user=user)
